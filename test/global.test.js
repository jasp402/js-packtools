...
describe('TEST # 1 - Test of integrity all functions', function () {
    ...
    it('Test, function objectToDate()',function () {
       ...
       assert.strictEqual(typeof u.objectToDate(strTimeA), 'object');
        // Logic: if [strTime] is {string}, then return: Object with date details.
       assert.strictEqual(typeof u.objectToDate(strTimeB), 'object');
        // Logic: if [strTime] is {string} but invalidate format, then return: string.
        assert.strictEqual(typeof u.objectToDate(strTimeC), 'string'); //=> 'Invalid Date'
    });
    ...
    it('Test, function validateYear()',function () {
        ...
        assert.strictEqual(u.validateYear(date), true);
        assert.strictEqual(u.validateYear('05/01/1994'), false);
        // Logic: if [date] is equal to set year [yearCompare], then is: true
        assert.strictEqual(u.validateYear('05/01/2019', '2019'), true);
        assert.strictEqual(u.validateYear('05/01/2019', '2018'), false);
        // Logic: if [date] is [less than / greater than] according to the [operator] according to set year [yearCompare], then is: true
        assert.strictEqual(u.validateYear('05/01/2019', '2020', '<'), true);
        assert.strictEqual(u.validateYear('05/01/2020', '2020', '<'), false);
        assert.strictEqual(u.validateYear('05/01/2000', '1997', '>'), true);
        assert.strictEqual(u.validateYear('05/01/1996', '1997', '>'), false);
        // Logic: if any [date] is different  of set year [yearCompare] according to the [operator] {!=}, then is: true
        assert.strictEqual(u.validateYear('05/01/2009', '2010', '!='), true);
        assert.strictEqual(u.validateYear('05/01/2010', '2010', '!='), false);
        assert.strictEqual(u.validateYear('05/01/2011', '2010', '!='), true);
    });
